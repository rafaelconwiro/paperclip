...
export function Agents() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={t("Select a company to view agents.")} />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{filtered.length === 1 ? t("{count} agent", { count: filtered.length }) : t("{count} agents", { count: filtered.length })}</p>
      )}
