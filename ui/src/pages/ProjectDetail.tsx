...
function ProjectWorkspacesContent({
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("No non-default workspace activity yet.")}</p>;
  }
      ...
      >
          ...
          <div className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
            ...
            {summary.executionWorkspaceStatus && summary.executionWorkspaceStatus !== "active" ? (
              <span className="text-[11px] text-muted-foreground">
                {t(summary.executionWorkspaceStatus.replace(/_/g, " "))}
              </span>
            ) : null}
          </div>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2">
            ...
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("Stop") : t("Start")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("Retry close") : t("Close")}
              </Button>
            ) : null}
          </div>
        </div>
        ...
        <div className="mt-1.5 space-y-0.5 text-xs text-muted-foreground">
          ...
          {summary.cwd ? (
            <div className="flex items-center gap-1.5">
              ...
              <CopyText text={summary.cwd} className="shrink-0" copiedLabel={t("Path copied")}>
        ...
        {summary.issues.length > 0 ? (
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
            <span className="font-medium text-muted-foreground/70">{t("Issues")}</span>
            ...
            {hiddenIssueCount > 0 ? (
              <Link to={workspaceHref} className="hover:text-foreground hover:underline">
                {t("+{count} more", { count: hiddenIssueCount })}
              </Link>
            ) : null}
          </div>
        ) : null}
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("Project");
      if (archived) {
        pushToast({ title: t("{name} has been archived", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("{name} has been unarchived", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("Failed to archive project") : t("Failed to unarchive project"),
        tone: "error",
      });
    },
  });
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("Project"),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("Issues") },
            { value: "overview", label: t("Overview") },
            ...(showWorkspacesTab ? [{ value: "workspaces", label: t("Workspaces") }] : []),
      ...
      {activeTab === "workspaces" ? (
        workspaceTabDecisionLoaded ? (
          ...
          <p className="text-sm text-muted-foreground">{t("Loading workspaces...")}</p>
        )
      ) : null}
