      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("{count} item{suffix}", { count: conflicts.length, suffix: conflicts.length === 1 ? "" : "s" })}
          </span>
        </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("{count} agent{suffix}", { count: agents.length, suffix: agents.length === 1 ? "" : "s" })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    t("Upload a .zip exported directly from Paperclip. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.");
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("No source configured."));
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("Failed to preview import."),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: t("{name}: {count} agent{suffix} processed.", {
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("Failed to apply import."),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("Package read failed"),
        body: err instanceof Error ? err.message : t("Failed to read folder."),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("Select a company to import into.")} />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {t("Choose a GitHub repo or upload a local Paperclip zip package.")}
          </p>
        </div>
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {(
            [
              { key: "github", icon: Github, label: t("GitHub repo") },
              { key: "local", icon: Upload, label: t("Local zip") },
            ] as const
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label={t("GitHub URL")}
            hint={t("Repo tree path or blob URL to COMPANY.md (e.g. github.com/owner/repo/tree/main/company).")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder={t("https://github.com/owner/repo/tree/main/company")}
              onChange={(e) => {
        ...
        <Field label={t("Target")} hint={t("Import into this company or create a new one.")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">{t("Create new company")}</option>
            <option value="existing">
              {t("Existing company: {name}", { name: selectedCompany?.name ?? t("None") })}
            </option>
          </select>
        </Field>
        ...
        {targetMode === "new" && (
          <Field
            ...
            hint={t("Optional override. Leave blank to use the package name.")}
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              ...
              placeholder={t("Imported Company")}
            />
          </Field>
        )}

        <Field
          label={t("Collision strategy")}
          hint={t("Board imports can rename, skip, or replace matching company content.")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
          ...
          >
            <option value="rename">{t("Rename on conflict")}</option>
            <option value="skip">{t("Skip on conflict")}</option>
            <option value="replace">{t("Replace existing")}</option>
          </select>
        </Field>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {previewMutation.isPending ? t("Previewing...") : t("Preview import")}
          </Button>
        </div>
      </div>
