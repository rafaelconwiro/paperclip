...
function FinanceSummaryCard({
  ...
  const eventCountLabel =
    eventCount === 1
      ? t("{count} total event in range", { count: eventCount })
      : t("{count} total events in range", { count: eventCount });
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">{t("Finance ledger")}</CardTitle>
        <CardDescription>
          {t("Account-level charges that do not map to a single inference request.")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle={t("Refunds, offsets, and credit returns")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("Net")}
          value={formatCents(netCents)}
          subtitle={t("Debit minus credit for the selected period")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("Estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("Estimated debits that are not yet invoice-authoritative")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("Select a company to view costs.")} />;
  }
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("Inference spend, platform fees, credits, and live quota windows.")}
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("Inference spend")}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("{tokens} tokens across request-scoped events", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("Budget")}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("{agents} agents paused · {projects} projects paused", {
                      agents: budgetData?.pausedAgentCount ?? 0,
                      projects: budgetData?.pausedProjectCount ?? 0,
                    })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("{spend} of {budget}", {
                        spend: formatCents(spendData.summary.spendCents),
                        budget: formatCents(spendData.summary.budgetCents),
                      })
                    : t("No monthly cap configured")
              }
              icon={Coins}
            />
            <MetricTile
              ...
              subtitle={t("{debits} debits · {credits} credits", {
            ...
            />
            <MetricTile
              label={t("Finance events")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("{estimated} estimated in range", {
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("Select a start and end date to load data.")}</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("Inference ledger")}</CardTitle>
                    <CardDescription>
                      {t("Request-scoped inference spend for the selected period.")}
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("Budget {budget}", { budget: formatCents(spendData.summary.budgetCents) })
                            : t("Unlimited budget")}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("usage")}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("{percent}% of monthly budget consumed in this range.", {
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">{t("By agent")}</CardTitle>
                    <CardDescription>{t("What each agent consumed in the selected period.")}</CardDescription>
                  </CardHeader>
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? t("{count} api", { count: row.apiRunCount }) : t("0 api")}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? t("{count} subscription", { count: row.subscriptionRunCount })
                                      : t("0 subscription")}
                                  </div>
                                ) : null}
                              </div>
                                    ...
                                    >
                                      <div className="min-w-0">
                                        ...
                                        <div className="truncate text-muted-foreground">
                                          {providerDisplayName(modelRow.biller)} · {t(billingTypeDisplayName(modelRow.billingType))}
                                        </div>
                                      </div>
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("{tokens} tok", {
                                            tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens),
                                          })}
                                        </div>
                                      </div>
                                    </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("By project")}</CardTitle>
                      <CardDescription>{t("Run costs attributed through project-linked issues.")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">{t("Budget control plane")}</CardTitle>
                  <CardDescription>
                    {t("Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers.")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle={t("Open soft or hard threshold crossings")}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("Pending approvals")}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("Budget override approvals awaiting board action")}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("Paused agents")}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("Agent heartbeats blocked by budget")}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("Paused projects")}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("Project execution blocked by budget")}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("Active incidents")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("Resolve hard stops here by raising the budget or explicitly keeping the scope paused.")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t("{scope} budgets", { scope: t(scopeType) })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("Company-wide monthly policy.")
                            : scopeType === "agent"
                              ? t("Recurring monthly spend policies for individual agents.")
                              : t("Lifetime spend policies for execution-bound projects.")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("No budget policies yet. Set agent and project budgets from their detail pages, or use the existing company monthly budget control.")}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("No cost events in this period.")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("No billable events in this period.")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("By biller")}</CardTitle>
                      <CardDescription>{t("Account-level financial events grouped by who charged or credited them.")}</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("No finance events yet.")}</p>
                      ) : (
