        ...
        <>
                ...
                <div>
                  <p className="text-sm font-medium text-red-50">
                    {data.budgets.activeIncidents === 1
                      ? t("{count} active budget incident", { count: data.budgets.activeIncidents })
                      : t("{count} active budget incidents", { count: data.budgets.activeIncidents })}
                  </p>
                  <p className="text-xs text-red-100/70">
                    {t("{count} agents paused", { count: data.budgets.pausedAgents })} · {t("{count} projects paused", { count: data.budgets.pausedProjects })} · {t("{count} pending budget approvals", { count: data.budgets.pendingApprovals })}
                  </p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-1 sm:gap-2">
            <MetricCard
              ...
              description={
                <span>
                  {data.agents.running} {t("running")}{", "}
                  {data.agents.paused} {t("paused")}{", "}
                  {data.agents.error} {t("errors")}
                </span>
              }
            />
            <MetricCard
              ...
              description={
                <span>
                  {data.tasks.open} {t("open")}{", "}
                  {data.tasks.blocked} {t("blocked")}
                </span>
              }
            />
            <MetricCard
              ...
              description={
                <span>
                  {data.costs.monthBudgetCents > 0
                    ? t("{percent}% of {amount} budget", { percent: data.costs.monthUtilizationPercent, amount: formatCents(data.costs.monthBudgetCents) })
                    : t("Unlimited budget")}
                </span>
              }
            />
              ...
              description={
                <span>
                  {data.budgets.pendingApprovals > 0
                    ? t("{count} budget overrides awaiting board review", { count: data.budgets.pendingApprovals })
