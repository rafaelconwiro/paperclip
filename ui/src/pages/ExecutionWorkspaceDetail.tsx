...
function parseWorkspaceRuntimeJson(
  value: string,
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t ? t("Workspace runtime JSON must be a JSON object.") : "Workspace runtime JSON must be a JSON object.",
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : (t ? t("Invalid JSON.") : "Invalid JSON."),
    };
  }
}
...
function validateForm(
  ...
  if (repoUrl) {
    try {
      ...
      return t ? t("Repo URL must be a valid URL.") : "Repo URL must be a valid URL.";
    }
  }
...
export function ExecutionWorkspaceDetail() {
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("Failed to save execution workspace."));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, action) => {
      ...
      setRuntimeActionMessage(
        action === "stop"
          ? t("Runtime services stopped.")
          : action === "restart"
            ? t("Runtime services restarted.")
            : t("Runtime services started."),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("Failed to control runtime services."));
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("Loading workspace…")}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("Failed to load workspace")}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("Failed to build workspace update."));
      return;
    }
      ...
      <div className="mx-auto max-w-5xl space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-wrap items-center gap-3">
          <Button variant="ghost" size="sm" asChild>
            <Link to={project ? `/projects/${projectRef}/workspaces` : "/projects"}>
              ...
              {t("Back to all workspaces")}
            </Link>
          </Button>
          <StatusPill>{t(workspace.mode.replace(/_/g, " "))}</StatusPill>
          <StatusPill>{t(workspace.providerType.replace(/_/g, " "))}</StatusPill>
          <StatusPill className={workspace.status === "active" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : undefined}>
            {t(workspace.status.replace(/_/g, " "))}
          </StatusPill>
        </div>
        ...
        <div className="grid gap-4 sm:gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.95fr)]">
          <div className="min-w-0 space-y-4 sm:space-y-6">
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
                <div className="min-w-0 space-y-2">
                  ...
                  <p className="max-w-2xl text-sm text-muted-foreground">
                    {t("Configure the concrete runtime workspace that Paperclip reuses for this issue flow.")}
                    <span className="hidden sm:inline">
                      {t("These settings stay attached to the execution workspace so future runs can keep local paths, repo refs, provisioning, teardown, and runtime-service behavior in sync with the actual workspace being reused.")}
                    </span>
                  </p>
                </div>
                ...
                >
                    {workspace.status === "cleanup_failed" ? t("Retry close") : t("Close workspace")}
                  </Button>
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("Workspace name")}>
                  <input
                    ...
                    placeholder={t("Execution workspace name")}
                  />
                </Field>
                <Field label={t("Branch name")} hint={t("Useful for isolated worktrees")}>
                  <input
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Field label={t("Working directory")}>
                  <input
                    ...
                    placeholder={t("/absolute/path/to/workspace")}
                  />
                </Field>
                <Field label={t("Provider path / ref")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.providerRef}
                    onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                    placeholder={t("/path/to/worktree or provider ref")}
                  />
                </Field>
              </div>

              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Field label={t("Repo URL")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Field label={t("Provision command")} hint={t("Runs when Paperclip prepares this execution workspace")}>
                  <textarea
                ...
                <Field label={t("Teardown command")} hint={t("Runs when the execution workspace is archived or cleaned up")}>
                  <textarea
                    className="min-h-20 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none sm:min-h-28"
              ...
              <div className="mt-4 grid gap-4">
                <Field label={t("Cleanup command")} hint={t("Workspace-specific cleanup before teardown")}>
                  <textarea
                ...
                <Field label={t("Runtime services JSON")} hint={t("Concrete workspace runtime settings for this execution workspace. Leave this inheriting unless you need a one-off override. If you are missing the right commands, ask your CEO to set them up for you.")}>
                  <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    <label htmlFor="inherit-runtime-config">{t("Inherit project workspace runtime config")}</label>
                  </div>
                  <textarea
              ...
              <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("No unsaved changes.")}</p> : null}
              </div>
            </div>
          </div>
          ...
          <div className="min-w-0 space-y-4 sm:space-y-6">
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("Linked objects")}</div>
                <h2 className="text-lg font-semibold">{t("Workspace context")}</h2>
              </div>
              ...
              <DetailRow label={t("Project workspace")}>
              ...
              <DetailRow label={t("Source issue")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("Derived from")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("Workspace ID")}>
            ...
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("Paths and refs")}</div>
                <h2 className="text-lg font-semibold">{t("Concrete location")}</h2>
              </div>
              <Separator className="my-4" />
              <DetailRow label={t("Working dir")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("None")}
              </DetailRow>
              <DetailRow label={t("Provider ref")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("None")}
              </DetailRow>
              <DetailRow label={t("Repo URL")}>
              ...
              <DetailRow label={t("Last used")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
            ...
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1">
                  <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("Runtime services")}</div>
                  <h2 className="text-lg font-semibold">{t("Attached services")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("Source:")} {runtimeConfigSource === "execution_workspace"
                      ? t("execution workspace override")
                      : runtimeConfigSource === "project_workspace"
                        ? t("project workspace default")
                        : t("none")}
                  </p>
                </div>
                    ...
                    <div key={service.id} className="rounded-xl border border-border/80 bg-background px-3 py-2">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-muted-foreground">{t(service.status.replace(/_/g, " "))} · {t(service.lifecycle.replace(/_/g, " "))}</div>
                          <div className="space-y-1 text-xs text-muted-foreground">
                            ...
                            {service.port ? <div>{t("Port {port}", { port: service.port })}</div> : null}
                            {service.command ? <MonoValue value={service.command} copy /> : null}
                            {service.cwd ? <MonoValue value={service.cwd} copy /> : null}
                          </div>
                        </div>
                        <StatusPill className="self-start">{t(service.healthStatus.replace(/_/g, " "))}</StatusPill>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("Recent operations")}</div>
                <h2 className="text-lg font-semibold">{t("Runtime and cleanup logs")}</h2>
              </div>
              <Separator className="my-4" />
              {workspaceOperationsQuery.isLoading ? (
                <p className="text-sm text-muted-foreground">{t("Loading workspace operations…")}</p>
              ) : workspaceOperationsQuery.error ? (
                <p className="text-sm text-destructive">
                  {workspaceOperationsQuery.error instanceof Error
                    ? workspaceOperationsQuery.error.message
                    : t("Failed to load workspace operations.")}
                </p>
                  ...
                  {workspaceOperationsQuery.data.slice(0, 6).map((operation) => (
                    <div key={operation.id} className="rounded-xl border border-border/80 bg-background px-3 py-2">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <StatusPill className="self-start">{t(operation.status.replace(/_/g, " "))}</StatusPill>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">{t("No workspace operations have been recorded yet.")}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("Linked issues")}</div>
              <h2 className="text-lg font-semibold">{t("Issues using this workspace")}</h2>
              <p className="text-sm text-muted-foreground">
                {t("Any issue attached to this execution workspace appears here so you can review the full session context before reusing or closing it.")}
              </p>
            </div>
            <StatusPill>{t("{count} linked", { count: linkedIssues.length })}</StatusPill>
          </div>
          <Separator className="my-4" />
          {linkedIssuesQuery.isLoading ? (
            <p className="text-sm text-muted-foreground">{t("Loading linked issues…")}</p>
          ) : linkedIssuesQuery.error ? (
            <p className="text-sm text-destructive">
              {linkedIssuesQuery.error instanceof Error
                ? linkedIssuesQuery.error.message
                : t("Failed to load linked issues.")}
            </p>
              ...
              {linkedIssues.map((issue) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="uppercase tracking-[0.16em]">{t(issue.priority)}</span>
            ...
            <p className="text-sm text-muted-foreground">{t("No issues are currently linked to this execution workspace.")}</p>
          )}
        </div>
      </div>
