    ...
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("Next: {time}", { time: new Date(trigger.nextRunAt).toLocaleString() })
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("Signing mode")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("Replay window (seconds)")}</Label>
                <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("Last: {value}", { value: trigger.lastResult })}</span>}
...
export function RoutineDetail() {
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({ title: t("{label} copied", { label }), tone: "success" });
    } catch (error) {
      pushToast({
        title: t("Failed to copy {label}", { label: label.toLowerCase() }),
        body: error instanceof Error ? error.message : t("Clipboard access was denied."),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not save the routine."),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("Routine run started"), tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not start the routine run."),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("Routine saved"),
        body: status === "paused" ? t("Automation paused.") : t("Automation enabled."),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not update the routine."),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not create the trigger."),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("Trigger saved"),
        body: t("The routine cadence update was saved."),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not update the trigger."),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not delete the trigger."),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("Webhook secret rotated"),
        webhookUrl: result.secretMaterial.webhookUrl,
        webhookSecret: result.secretMaterial.webhookSecret,
      });
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("Paperclip could not rotate the webhook secret."),
        tone: "error",
      });
    },
  });
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("Routine not found")}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived" ? t("Archived") : automationEnabled ? t("Active") : t("Paused");
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex shrink-0 items-center gap-3 pt-1">
          ...
          <ToggleSwitch
            ...
            aria-label={automationEnabled ? t("Pause automatic triggers") : t("Enable automatic triggers")}
          />
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("Save this now. Paperclip will not show the secret value again.")}</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("Webhook URL"), secretMessage.webhookUrl)}>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("Webhook secret"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("Secret")}
              </Button>
            </div>
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("Advanced delivery settings")}</span>
              ...
              >
                ...
                <SelectContent>
                  {concurrencyPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{t(value.replaceAll("_", " "))}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{t(concurrencyPolicyDescriptions[editDraft.concurrencyPolicy])}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("Catch-up")}</p>
              <Select
              ...
              <p className="text-xs text-muted-foreground">{t(catchUpPolicyDescriptions[editDraft.catchUpPolicy])}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("Add trigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {t(kind)}{kind === "webhook" ? ` - ${t("COMING SOON")}` : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{t(signingModeDescriptions[newTrigger.signingMode])}</p>
                  </div>
            ...
            <div className="flex items-center justify-end">
              <Button size="sm" onClick={() => createTrigger.mutate()} disabled={createTrigger.isPending}>
                {createTrigger.isPending ? t("Adding...") : t("Add trigger")}
              </Button>
            </div>
          </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("No triggers configured yet.")}</p>
          ) : (
              ...
              {(routineRuns ?? []).map((run) => (
                <div key={run.id} className="flex items-center justify-between px-3 py-2 text-sm">
                  <div className="flex items-center gap-2 min-w-0">
                    <Badge variant="outline" className="shrink-0">{t(run.source)}</Badge>
