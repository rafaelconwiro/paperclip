        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1 text-xs">
              {row.eventCount === 1
                ? t("{eventCount} event across {kindCount} kind", {
                    eventCount: row.eventCount,
                    kindCount: row.kindCount,
                  })
                : t("{eventCount} events across {kindCount} kinds", {
                    eventCount: row.eventCount,
                    kindCount: row.kindCount,
                  })}
            </CardDescription>
          ...
          <div className="text-right">
            ...
            <div className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground">{t("net")}</div>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-3 px-4 pb-4 pt-3">
        <div className="grid gap-2 text-sm sm:grid-cols-3">
          <div className="border border-border p-3">
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("debits")}</div>
            <div className="mt-1 font-medium tabular-nums">{formatCents(row.debitCents)}</div>
          </div>
          <div className="border border-border p-3">
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("credits")}</div>
            <div className="mt-1 font-medium tabular-nums">{formatCents(row.creditCents)}</div>
          </div>
          <div className="border border-border p-3">
            <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("estimated")}</div>
