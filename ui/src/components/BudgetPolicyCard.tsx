...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (key: string, params?: Record<string, string | number>) => string) {
  return windowKind === "lifetime" ? t("Lifetime budget") : t("Monthly UTC budget");
}
...
function statusLabel(
  summary: Pick<BudgetPolicySummary, "paused" | "status">,
  t: (key: string, params?: Record<string, string | number>) => string,
) {
  if (summary.paused) return t("Paused");
  if (summary.status === "warning") return t("Warning");
  if (summary.status === "hard_stop") return t("Hard stop");
  return t("Healthy");
}
...
export function BudgetPolicyCard({
  ...
  const pauseDetail =
    summary.paused && summary.pauseReason
      ? ` · ${t("{reason} pause", { reason: t(summary.pauseReason) })}`
      : "";

  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("Observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0
            ? t("{percent}% of limit", { percent: summary.utilizationPercent })
            : t("No cap configured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("Budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("Disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("Soft alert at {percent}%{pauseDetail}", {
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("Unlimited")}</span>
      </div>
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("Saving...") : summary.amount > 0 ? t("Update budget") : t("Set budget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("Enter a valid non-negative dollar amount.")}</p>
        ) : null}
      </div>
    );
  }
