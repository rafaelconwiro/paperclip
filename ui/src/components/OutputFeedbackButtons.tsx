    ...
    <>
      ...
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("What could have been better?")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("Add a short note")}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("Save your feedback sharing preference")}</DialogTitle>
            <DialogDescription>
              {t("Choose whether voted AI outputs can be shared with Paperclip Labs. This answer becomes the default for future thumbs up and thumbs down votes.")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>{t("This vote is always saved locally.")}</p>
            <p>
              {t("Choose ")}
              <span className="font-medium text-foreground">{t("Always allow")}</span>
              {t(" to share this vote and future voted AI outputs. Choose ")}
              <span className="font-medium text-foreground">{t("Don't allow")}</span>
              {t(" to keep this vote and future votes local.")}
            </p>
            <p>
              {t("You can change this later in Instance Settings > General.")}
            </p>
            {termsUrl ? (
              <a
                href={termsUrl}
          ...
          <DialogFooter>
            ...
            >
              {isSaving ? t("Saving...") : t("Don't allow")}
            </Button>
            <Button
              type="button"
              disabled={!pendingVote || isSaving}
            ...
            >
              {isSaving ? t("Saving...") : t("Always allow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
